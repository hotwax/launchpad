...
export default defineComponent({
  ...
  methods: {
    redirectToUserDetails() {
      window.location.href = `${this.authStore.isMoquiOnly ? process.env.VUE_APP_COMPANY_LOGIN_URL : process.env.VUE_APP_USERS_LOGIN_URL}?oms=${this.authStore.oms}&token=${this.authStore.token.value}&expirationTime=${this.authStore.token.expiration}&partyId=${this.authStore.current.partyId}&omsRedirectionUrl=${this.authStore.maargOms}`
